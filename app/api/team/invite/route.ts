...
const INVITE_FIELDS = "id, email, full_name, role, status, token, expires_at, created_at";
  ...
  try {
    ...
    if (existing) {
      const { data, error } = await supabase
        .from("company_invites")
        .update({ full_name: fullName, role, expires_at: expiresAt })
        .eq("id", existing.id)
      ...
      const { data, error } = await supabase
        .from("company_invites")
        .insert({ company_id: companyId, email, full_name: fullName, role, invited_by: user.id, expires_at: expiresAt })
        .select(INVITE_FIELDS)
        .single();
      assertNoDbError(error);
      invite = data;
    ...
    const send = await sendInviteEmail({ email, redirectTo, fullName });
